...
import SelectInput from './DataTable/components/SelectInput';
...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  const onClickColumnFilterButton = useCallback(() => {
  ...
  const getColumnConfigs = useCallback(
    (
      ...
      i: number,
    ): ColumnWithLooseAccessor<D> & UseFiltersColumnOptions<D> => {
      const {
        key,
        label,
        isNumeric,
      ...
      return {
        ...
        Header: ({
          ...
          >
            ...
            {includeSearch ? (
              ...
              >
                {col.canFilter && showFilter ? col.render('Filter') : null}
              </div>
            ) : null}
          </th>
        ),
        ...
        Filter: ({ column: { filterValue, preFilteredRows, setFilter } }) => {
          ...
          preFilteredRows.forEach(row => options.add(row.values[i] as string));
          return (
            // <input
            //   value={filterValue || ''}
            //   onClick={e => e.stopPropagation()}
  ...
  return (
    <Styles>
      <DataTable<D>
        ...
        onClickColumnFilterButton={onClickColumnFilterButton}
      />
    </Styles>
  );
}
